...
# Actual max S = 35+12+7+5+4+10+13+2 = 88.0  (gender=5, EXP_BONUS Trauma=10)
...
print(f"\nSweep: EXP_BONUS={EXP_BONUS}, gender=5, S_MIN={S_MIN}, actual_max=87.0")
print(f"\n{'S_MAX':>6} | {'Model':<10} | {'Acc':>7} | {'F1':>7} | {'AUC':>7} | {'Balance':>7}")
print("-" * 57)

records = []
for s_max in range(87, 79, -1):
    df = generate_dataset(s_max)
    metrics, bal = train_and_eval(df)
    for model_name, m in metrics.items():
        print(f"  {s_max:>4} | {model_name:<10} | {m['Acc']:>7.4f} | {m['F1']:>7.4f} | {m['AUC']:>7.4f} | {bal:>7.3f}")
        records.append({"S_MAX": s_max, "Model": model_name, **m, "Balance": bal})
