...
from utils.db import load_counselors, save_intro_request
...
def inject_styles():
    st.markdown(
        ...
        .form-card {
            background:
                radial-gradient(1200px 300px at -10% -30%, rgba(139,92,246,0.08) 0%, transparent 60%),
                radial-gradient(900px 260px at 110% 120%, rgba(109,40,217,0.06) 0%, transparent 55%),
                #FFFFFF;
            border-radius: 24px;
        ...
        /* ── Step header (number chip + serif title) ──────────────────── */
        .step-header {
            display: flex;
            align-items: center;
            gap: 14px;
        ...
        .section-divider {
            ...
            margin: 0px 0 20px 0 !important; /* Balanced top and bottom spacing */
            position: relative;
            z-index: 10;
        }
        /* Tighten Streamlit's flex gap between stacked widgets on the matching page */
        [data-testid="block-container"] [data-testid="stVerticalBlock"] {
            gap: 0.5rem !important;
        }
        /* Kill internal bottom space on radio/number/select widgets */
...
def render_step_progress(step: int, total: int = 3):
    ...
    for i, name in enumerate(step_names, 1):
        if i < step:
            ...
            circle_color = "#FFFFFF"
            circle_border = "none"
            shadow = "0 6px 16px rgba(124,58,237,0.40), 0 0 0 4px rgba(139,92,246,0.15)"
            label_color = "#1A1A2E"
            weight = "700"
            inner = str(i)
        else:
            ...
            circle_bg = "#FFFFFF"
            circle_color = "#B0A9A0"
            circle_border = "1.5px solid #E5E0D8"
            shadow = "none"
            label_color = "#B0A9A0"
            weight = "500"
            inner = str(i)
        ...
        nodes_html += (
            ...
            f'font-family:\'DM Sans\',sans-serif;font-weight:700;font-size:13px;box-shadow:{shadow};'
            f'transition:all 0.25s ease;">{inner}</div>'
            f'<span style="font-size:11.5px;color:{label_color};font-weight:{weight};letter-spacing:0.02em;">{name}</span>'
            f'</div>'
        )

    # Connecting line fill percentage — center of step 1 to center of current step
    if total > 1:
        fill_pct = max(0, min(100, int((step - 1) / (total - 1) * 100)))
    else:
        fill_pct = 0
    ...
    st.markdown(
        ...
        <div style="position:relative;padding:4px 8px 8px;margin-bottom:30px;">
            <!-- Connecting track -->
            <div style="position:absolute;left:calc(8px + 17px);right:calc(8px + 17px);top:calc(4px + 17px);
                        height:2px;background:#EDE8E3;border-radius:2px;z-index:1;"></div>
            <!-- Filled portion -->
            <div style="position:absolute;left:calc(8px + 17px);top:calc(4px + 17px);width:calc((100% - 16px - 34px) * {fill_pct} / 100);
                        height:2px;background:linear-gradient(90deg,#8B5CF6,#6D28D9);border-radius:2px;z-index:1;
...
def render_step_header(number: int, title: str, subtitle: str):
    st.markdown(
        ...
        <div class="step-header">
            <div class="step-chip">{number}</div>
...
def show_profile_dialog(counselor, score=None):
    c = counselor
    ...
    # Use stored expertise_tags if available, else auto-generate
    if ext.strip():
        pills = [t.strip() for t in ext.split(",") if t.strip()][:6]
    else:
        pills = _expertise_tags(spec, modality)

    # Use stored helpful thoughts if available, else defaults
    thoughts = [ht1, ht2]
    if not any(t.strip() for t in thoughts):
        thoughts = _helpful_thoughts(spec)
    ...
    st.markdown(
        ...
        <div style="display:flex;align-items:center;gap:24px;margin-bottom:20px;">
            <img src="{avatar_url}" width="90" height="90"
                 style="border-radius:16px;border:2px solid #EDE8E3;flex-shrink:0;" />
            <div style="flex:1;">
    ...
    st.markdown(
        f"""
        <div style="background:#F8F5FF; border:1px solid #E9E1FF;
                    border-radius:14px;padding:20px 22px;margin-bottom:16px;">
            <p style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;
                      color:#8B5CF6;margin:0 0 12px;">Thoughts I can help you with</p>
            {thought_items}
        </div>
        """,
        unsafe_allow_html=True,
...
def show_matching_page():
    ...
    # Ensure all form variables are initialized so they persist across wizard steps
    if "client_age" not in st.session_state: st.session_state.client_age = 25
    if "client_gender" not in st.session_state: st.session_state.client_gender = sorted_options(df_ref["client_gender"])[0] if sorted_options(df_ref["client_gender"]) else ''
    if "client_ethnicity" not in st.session_state: st.session_state.client_ethnicity = sorted_options(df_ref["client_ethnicity"])[0] if sorted_options(df_ref["client_ethnicity"]) else ''
    if "client_issue" not in st.session_state: st.session_state.client_issue = sorted_options(df_ref["client_issue"])[0] if sorted_options(df_ref["client_issue"]) else ''
    if "previous_exp" not in st.session_state: st.session_state.previous_exp = 0
    if "preferred_modality" not in st.session_state: st.session_state.preferred_modality = sorted_options(df_ref["preferred_modality"])[0] if sorted_options(df_ref["preferred_modality"]) else ''
    if "preferred_language" not in st.session_state: st.session_state.preferred_language = sorted_options(df_ref["preferred_language"])[0] if sorted_options(df_ref["preferred_language"]) else ''
    if "preferred_c_gender" not in st.session_state: st.session_state.preferred_c_gender = sorted_options(df_ref["preferred_counselor_gender"])[0] if sorted_options(df_ref["preferred_counselor_gender"]) else ''
    ...
    # ── WIZARD STEP 4: Results & ML Processing ──────────────────────────────────
        ...
        # ── Style "View Profile" buttons to fuse with cards ───────────────────────
        components.html(
            """
            <script>
            ...
            function styleVPButtons() {
                ...
                cols.forEach(function(col) {
                    ...
                    // Zero out Streamlit spacing between card and button
                    col.querySelectorAll('[data-testid="element-container"]').forEach(function(ec) {
                        sp(ec, 'margin-top', '0');
                        sp(ec, 'padding-top', '0');
                        sp(ec, 'margin-bottom', '0');
                        sp(ec, 'padding-bottom', '0');
                    });
                    ...
                    col.querySelectorAll('[data-testid="stButton"]').forEach(function(wrapper) {
                        sp(wrapper, 'width', '100%');
                        sp(wrapper, 'margin', '0');
                        sp(wrapper, 'padding', '0');
                        var btn = wrapper.querySelector('button');
                        if (!btn) return;
                        sp(btn, 'background', bg);
                        sp(btn, 'color', color);
                        sp(btn, 'border-radius', '14px');
                        sp(btn, 'border', 'none');
                        sp(btn, 'width', '100%');
                        sp(btn, 'padding', '13px 24px');
                        sp(btn, 'font-size', '12px');
                        sp(btn, 'font-weight', '600');
                        sp(btn, 'letter-spacing', '0.05em');
                        sp(btn, 'text-transform', 'uppercase');
                        sp(btn, 'box-shadow', 'none');
                        sp(btn, 'transform', 'none');
                        btn.onmouseenter = function() {
                            sp(btn, 'background', hoverBg);
                            sp(btn, 'color', hoverColor);
                        };
            ...
            styleVPButtons();
            setTimeout(styleVPButtons, 150);
            setTimeout(styleVPButtons, 500);
            </script>
            """,
            height=0,
            width=0,
